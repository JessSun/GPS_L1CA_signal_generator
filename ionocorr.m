function ionodel = ionocorr(systime,svxyz,usrxyz,ionoutc)
% Compute ionospheric correction from the broadcast model parameters
%
% ionodel = ionocorr(systime,svxyz,usrxyz)
%
% INPUTS:
%	systime = time at which iono delay is to be computed; note this is 
%   expressed as system time (i.e., GPS time of week in seconds)
...
%	NOTE: This function assumes the iono broadcast model parameters 
%   (ALPHA and BETA vectors) have been loaded into global memory
%
% OUTPUTS:
...
PER = ionoutc.beta0 + ionoutc.beta1*phim + ionoutc.beta2*phim^2 + ionoutc.beta3*phim^3;
if PER < 72000, PER = 72000; end
x = 2*pi*(t - 50400)/PER;
	
AMP = ionoutc.alpha0 + ionoutc.alpha1*phim + ionoutc.alpha2*phim^2 + ionoutc.alpha3*phim^3;
if AMP < 0, AMP = 0; end
